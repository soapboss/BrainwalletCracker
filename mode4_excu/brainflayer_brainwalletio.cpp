...
public:
    // Generate private key using BrainwalletIO algorithm (scrypt + SHA-256)
    static std::vector<unsigned char> generatePrivKeyBrainwalletIO(const std::string& passphrase, const std::string& salt) {
        std::vector<unsigned char> privkey(32); // 32 bytes for private key
        
        // BrainwalletIO algorithm:
        // 1. Generate seed using scrypt
        // 2. Convert seed to hex string
        // 3. Hash the hex string with SHA-256 to get final private key
        
        // Step 1: Generate seed using scrypt
        ...
        for (int i = 0; i < 32; i++) {
            hexStream << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(seed[i]);
        }
        std::string hexSeed = hexStream.str();
        
        // Step 3: Hash the hex string with SHA-256
        SHA256_CTX sha256;
        SHA256_Init(&sha256);
        SHA256_Update(&sha256, hexSeed.c_str(), hexSeed.length());
        SHA256_Final(privkey.data(), &sha256);
...
void processBatch(
    ...
    for (const auto& line : inputLines) {
        ...
        // Generate private key using BrainwalletIO
        auto privKey = BitcoinCrypto::generatePrivKeyBrainwalletIO(passphrase, salt);
